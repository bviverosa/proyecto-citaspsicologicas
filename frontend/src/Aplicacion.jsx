...
class Application extends React.Component {
  ...
  render() {
    ...
    // 1. CONDICIONAL: Si se hace clic en Consultorio, mostramos el login del psicólogo
    if (seccion === 'consultorio') {
      return <Intro />;
    }

    // 2. VISTA ORIGINAL DEL PACIENTE
    return (
      <div>
        {/* HEADER RESTAURADO A SU DISEÑO ORIGINAL */}
        <header>
          <div className="logo" onClick={() => this.irA('inicio')} style={{ cursor: 'pointer', color: 'white'}}>
            🌿 SerenaMente
          </div>
          <div 
            className="user-profile" 
            onClick={() => this.irA('consultorio')} 
        ...
        <footer>
          ...
          <div className="footer-links">
            <a onClick={() => this.irA('psico')} style={{ cursor: 'pointer' }}>Psicoeducación</a>
            <a onClick={() => this.irA('relax')} style={{ cursor: 'pointer' }}>Relajación</a>
            <a onClick={() => this.irA('recursos')} style={{ cursor: 'pointer' }}>Recursos</a>
          </div>
        </footer>
      </div>
    );
